...
- Shared application UX: reusable button, switch, number-field, badge, notice, skeleton, spinner/loading, container, status, and feedback primitives under `src/shared/react/` and `src/shared/styles/`. Viewer, Settings, and Popup compose these shared contracts while keeping surface-specific layout local.
...
- `src/viewer/navigation/link-resolver.js`
  - **`resolveMarkdownLink(rawHref, context)`** — pure function that classifies a raw `<a href>` into one of: `same-document-hash`, `self-link`, `document-file`, `workspace-virtual-file`, `external`, `asset`, `unsupported`. Registered Markdown-family, `.txt`, `.sql`, `.mermaid`, raster image, and SVG targets are interceptable. Returns `{ kind, resolvedUrl, hash, shouldIntercept }`.
...
- `src/viewer/explorer/explorer-state.js`
  - `sessionStorage`: original file URL, independent Files/Outline widths, **workspace root** `file:` URL, **mode** `sibling` | `workspace`, and explorer expanded-folder state keyed by mode/root.

- `src/content/host-print.scss` — host/light-DOM print rules (injected with viewer root); pairs with `viewer/styles/_chrome-print.scss` (Files panel/right rail hidden) and `viewer/styles/content/_article-print.scss` (article typography for print).
...
- TOC right rail with integrated document actions (original Phase 3 TOC, later separated from Files)
...
- **Multi-format viewer architecture** (`docs/multi-format-viewer-architecture.md`): Phases 0–7 completed. File-type registry, `.mdc`, document identity/session/loading/renderer-dispatch boundaries, capability-driven chrome, explorer-only plain `.txt`, dedicated highlighted `.sql`, raster images, stricter `<img>`-only SVG rendering, and standalone `.mermaid` rendered/raw views are live. Integrated navigation now coordinates refresh, active-file state, cross-folder scans, browser Back/Forward, focus, and format changes. Standalone and fenced Mermaid share the lazy render service, sanitizer, safe error UI, actions, lightbox, and renderer/theme mapping.
