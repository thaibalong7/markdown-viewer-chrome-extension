...
export const MAX_PLAIN_TEXT_BYTES = 5 * 1024 * 1024

export class DocumentTooLargeError extends Error {
  constructor(maxBytes = MAX_PLAIN_TEXT_BYTES) {
    super(`This text file is larger than the ${Math.floor(maxBytes / (1024 * 1024))} MiB viewing limit.`)
...
export async function loadDocument({ href, fileType, workspaceReader, signal }) {
  throwIfAborted(signal)
  if (!fileType) {
  ...
  if (workspaceReader) {
    const file = typeof workspaceReader.text === 'function'
      ...
      : await workspaceReader.getFile()
    throwIfAborted(signal)
    if (Number.isFinite(file?.size)) enforceStandaloneTextLimit(fileType, file.size)
  ...
  throwIfAborted(signal)
  enforceStandaloneTextLimit(fileType, utf8ByteLength(text))
