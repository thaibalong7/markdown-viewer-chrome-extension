...
- Hỗ trợ các syntax cơ bản:
  ...
  - image
  - link
  - internal Markdown link navigation (click to open linked `.md` files in-viewer)
...
**Ghi chú cập nhật (theo code 2026):** Phase **0–3**, **5–8**, **8.1**, **9** đã triển khai trong repo. **Phase 4** (drawer settings *trong trang*) được thay bằng **popup React** + `SETTINGS_UPDATED` — xem [`project-overview-for-ai.md`](./project-overview-for-ai.md). Viewer shell/TOC/Files là **React** (`src/viewer/react/`); các đường dẫn `viewer/shell/*`, `messaging/send-message.js`… dưới đây là spec lịch sử. **React migration** (chrome UI): [`react-migration-plan.md`](./react-migration-plan.md) — Phase **4-R** (drawer trong viewer) *deferred*. **Internal Markdown link navigation** đã hoàn thành — xem [`internal-hyperlink-navigation-solution.md`](./internal-hyperlink-navigation-solution.md). **Phase 10** (hardening/performance) vẫn mở.
...
### Trạng thái (repo)
✅ Hoàn thành — layout thực tế: `service-worker.js`, `messaging/index.js`, `settings/index.js`, v.v. (không dùng đúng tên file trong danh sách dưới).
...
### Trạng thái (repo)
✅ Hoàn thành — thêm `shared/markdown-detect.js` cho pathname / heuristic chung.
...
### Trạng thái (repo)
✅ Hoàn thành — shell là React (`ViewerShell.jsx`), sanitize nằm trong `renderer.js`.
...
### Trạng thái (repo)
✅ Hoàn thành — `OutlinePanel.jsx` + `useScrollSpy.js` (không còn `toc-sidebar.js` imperative).
...
### Trạng thái
✅ **Mục tiêu customization đạt được** qua **extension popup (React)** + `SAVE_SETTINGS` / `SETTINGS_UPDATED`. **Không** có settings drawer trong viewer như spec gốc; xem Phase **4-R** trong [`react-migration-plan.md`](./react-migration-plan.md) nếu làm drawer sau.

### Mục tiêu (spec gốc)
Làm nút Settings góc trên bên phải hoạt động đầy đủ (trong repo: cấu hình qua popup).
...
### Trạng thái (repo)
✅ Hoàn thành — `plugins/plugin-manager.js` + `plugin-types.js` (không có thư mục `plugins/registry/` riêng).
...
- `src/plugins/optional/mermaid-lightbox.js`
- `src/plugins/optional/mermaid-export.js`
- `src/viewer/styles/content.scss` (nút action cho block Mermaid)

### Deliverable
- Người dùng có thể export Mermaid chart thành `SVG/PNG` ngay trong trang đọc Markdown.
- Mermaid chart có lightbox toàn màn hình với:
  - mở từ chart hoặc nút expand
  - zoom in / zoom out / re-center / close
  - drag toàn màn hình để pan
  - pinch / `Ctrl + Scroll` để zoom
  - `Esc`, `+`, `-`, `0`
  - UI follow theme hiện tại
  - lightbox render chart ở mật độ cao hơn để zoom sắc nét hơn

---

## Phase 8.2 - Internal Markdown link navigation (DONE)

### Mục tiêu
Khi người dùng click hyperlink trong nội dung Markdown trỏ tới file Markdown khác (relative hoặc absolute), viewer mở file đó ngay bên trong Markdown Plus mà không reload trang.

### Kết quả đạt được
- Fix markdown engine: `link_open` chỉ gán `target="_blank"` cho external links, internal links giữ không có `target`
- Link resolver (`src/viewer/navigation/link-resolver.js`): phân loại link thành same-document-hash, self-link, markdown-file, workspace-virtual-file, external, asset, unsupported
- Click interception trong `article-interactions.js`: tôn trọng modifier keys (Ctrl/Cmd+click, middle click), `target`, `download` attrs
- Browser Back/Forward hoạt động qua `popstate` handler + `hashchange` coordination
- Sidebar active file sync khi navigate qua article link, re-scan folder khi vượt qua sibling root
- Virtual workspace relative links
- Hỗ trợ: spaces, Unicode, encoded hrefs, parent folder traversal, hash fragment navigation, self-link scroll
- Unit tests cho link-resolver

### File đã implement / modified
- `src/viewer/navigation/link-resolver.js` (new)
- `src/viewer/navigation/__tests__/link-resolver.test.js` (new)
- `src/viewer/core/markdown-engine.js` (modified — `link_open` rule)
- `src/viewer/article-interactions.js` (modified — internal link interception + `popstate` handler)
- `src/viewer/app.js` (modified — bridge wiring `resolveLink` + `navigateToFile`)
- `src/viewer/react/hooks/useExplorer.js` (modified — sidebar integration)
- `src/viewer/explorer/explorer-tree-utils.js` (modified)
- `src/viewer/styles/content/_typography.scss` (modified)

### Deliverable
- Người dùng click link Markdown nội bộ → mở file trong cùng viewer, giữ sidebar/theme/plugin, Back/Forward hoạt động tự nhiên.

### Thiết kế chi tiết
Xem [`internal-hyperlink-navigation-solution.md`](./internal-hyperlink-navigation-solution.md).
...
### Trạng thái (repo)
✅ **Popup React** đầy đủ (`PopupApp.jsx`, `panels/*`). **Options** là trang tối giản (`src/options/index.js`) — không có `options/sections/*` hay form đầy đủ như spec gốc.
...
### Trạng thái (repo)
⏳ Chưa hoàn thành — xem [`performance-issues-audit.md`](./performance-issues-audit.md) (baseline tối ưu).
